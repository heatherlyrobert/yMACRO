...
yMACRO_flatten_at       (uchar a_src, uchar a_dst)
{
   ...
   DEBUG_SCRP   yLOG_value   ("x_src"     , x_src);
   --rce; if (x_src < 0) { 
      DEBUG_SCRP   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   DEBUG_SCRP   yLOG_value   ("nkey"      , g_macros [x_src].len);
   ...
   DEBUG_SCRP   yLOG_value   ("x_dst"     , x_dst);
   --rce; if (x_dst < 0) { 
      DEBUG_SCRP   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
...
ymacro__install         (int a_lvl, int a_in, int a_len, int a_pos, int a_out)
{
   ...
   DEBUG_SCRP  yLOG_complex ("final"     , "%3d/%c, %3d, å%sæ", a_out, S_MACRO_LIST [a_out], x_len, x_keys);
   /*---(complete)-----------------------*/
   DEBUG_SCRP  yLOG_exit    (__FUNCTION__);
   return i;
}
